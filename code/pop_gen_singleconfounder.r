################################################################################################
# NAME: pop_gen_singleconfounder.r
# AUTHORS: Ellie Matthay, Catherine Li, Chris Rowe
# DATE STARTED: 5/19/20120   
...
#          Includes changes to original pop_gen.r to have only a single confounder. Purpose
#          of having only a single confounder is to for assessing performance of
#         matching estimators
################################################################################################
...
pop$A.50 <- exposure_gen(log(0.414)) # 50.03%
pop$A.20 <- exposure_gen(log(0.0814)) # 20.01%
pop$A.10 <- exposure_gen(log(0.0339)) # 10.01%
pop$A.05 <- exposure_gen(log(0.01557)) # 5.001%
...
outcome.20 <- outcome_gen(trueRate = log(2), 
                          BaselineHazard = c(log(0.00001175), log(0.00001508), log(0.00001680), log(0.00001778)),
                          expfreq = c("A.50", "A.20", "A.10", "A.05"), 
                          outcomefreq = "Y.20")

outcome.10 <- outcome_gen(trueRate = log(2), 
                          BaselineHazard = c(log(0.000005285), log(0.000006785), log(0.000007600), log(0.000008095)),
                          expfreq = c("A.50", "A.20", "A.10", "A.05"), 
                          outcomefreq = "Y.10")

outcome.02 <- outcome_gen(trueRate = log(2), 
                          BaselineHazard = c(log(0.000000979), log(0.000001249), log(0.000001403), log(0.000001497)),
                          expfreq = c("A.50", "A.20", "A.10", "A.05"), 
                          outcomefreq = "Y.02")

outcome.005 <- outcome_gen(trueRate = log(2), 
                           BaselineHazard = c(log(0.0000002385), log(0.0000003122), log(0.0000003465), log(0.0000003700)),
                           expfreq = c("A.50", "A.20", "A.10", "A.05"), 
                           outcomefreq = "Y.005")
...
write.csv(pop, "data/population.data.singleconfounder.csv", row.names=F)

# export true and crude measures
write.csv(TrueMeasures_IDR, "data/singleconfounder.trueIDR.csv", row.names=T)
write.csv(Crude_IDR, "data/singleconfounder.crudeIDR.csv", row.names=T)
