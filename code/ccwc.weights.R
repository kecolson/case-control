  ...
  for (g in fg) {
    ...
    for (tf in ft) {
      ...
      if (ncont>0) {
        ...
        if (is.null(weights)) {
          pr[(nn+ncase+1):(newnn)] <- sample((1:n)[noncase], size=ncont, replace = FALSE) #Replace is FALSE is no weights passed in
        } else {
          pr[(nn+ncase+1):(newnn)] <- sample((1:n)[noncase], size=ncont, replace = TRUE, #Replace is TRUE if weights passed in
